...
const getMoviesByTrendingAllDay = async () => {
    const endpoint = "https://api.themoviedb.org/3/trending/all/day?api_key=" + process.env.REACT_APP_MOVIE_DB_API;
    const result = await fetch(endpoint).then(response => {
        if (response.status === 200) {
            return response.json();
        } else {
...
const getTvShowByPopular = async () => {
    const endpoint = "https://api.themoviedb.org/3/tv/popular?api_key=" + process.env.REACT_APP_MOVIE_DB_API;
    const result = await fetch(endpoint).then(response => {
        if (response.status === 200) {
            return response.json();
        } else {
